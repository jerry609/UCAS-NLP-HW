...
class Trainer:
    ...
    def plot_training_history(self):
        ...
        plt.title('Training, Validation, and Test Loss', pad=15)
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        plt.grid(True)
        ...
        plt.title('Training, Validation, and Test Accuracy', pad=15)
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy (%)')
        plt.legend()
        plt.grid(True)
...
def main():
    ...
    optimizer = optim.Adam(model.model.fc.parameters(), lr=1e-4, weight_decay=1e-4)
